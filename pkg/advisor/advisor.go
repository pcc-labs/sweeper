...
func Advise(ctx context.Context, exec worker.Executor, dir string, tasks []planner.FixTask, histories map[string]loop.FileHistory, round int) (Plan, error) {
	...
	res := exec(ctx, worker.Task{
		...
		Dir:    dir,
		Prompt: BuildPrompt(tasks, histories, round),
	})
	if !res.Success {
